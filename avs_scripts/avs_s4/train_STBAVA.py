...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_weights", type=str, default='', help='path of trained model')
    ...
    scripts_to_save = ['train_STBAVA.py', 'config.py', 'dataloader.py', './model/STBAVA.py', './model/SAMA_Enc.py', 'loss.py']
    ...
    model.load_state_dict(torch.load(os.path.join(args.sam_weights, "sama_s4_best.pth")), strict=False)
    ...
    for epoch in range(args.max_epoches):
        ...
        for n_iter, batch_data in enumerate(train_dataloader):
            imgs, audio, mask, category, video_name = batch_data # imgs[bs, 5, 3, 224, 224], audio[bs, 5, 1, 96, 64], mask[bs, 1, 1, 224, 224]
            ...
            output = model(imgs, audio_feature=audio_feature) # [bs*5, 1, 224, 224]

            loss, loss_dict = IoULoss(output.squeeze(1), mask.unsqueeze(1))
            ...
            if (global_step-1) % 50 == 0:
                train_log = 'Iter:%5d/%5d, iou_loss:%.4f, lr: %.4f'%(
                            global_step-1, max_step, avg_meter_iou_loss.pop('iou_loss'), optimizer.param_groups[0]['lr'])
        ...
        with torch.no_grad():
            for n_iter, batch_data in enumerate(test_dataloader):
                imgs, audio, mask, category, video_name = batch_data # [bs, 5, 3, 224, 224], [bs, 5, 1, 96, 64], [bs, 5, 1, 224, 224]
                ...
                output = model(imgs, audio_feature=audio_feature, mode=5) # [bs*5, 1, 224, 224]
