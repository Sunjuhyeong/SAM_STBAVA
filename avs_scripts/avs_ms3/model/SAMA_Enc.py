...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        for i in range(depth):
            ...
            # if i < depth - self.blocks_inter_idx:
            #     self.blocks[i].to("cuda:1")
    ...
    def forward(self, x: torch.Tensor, spec: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_4gpus:
                if i < 9: 
                    x = x.to("cuda:3")
                    prompt[i] = prompt[i].to("cuda:3")
                    x = prompt[i].reshape(B, 1, 1, -1) + x
                    x = blk(x)

                elif 9 <= i < 18:
                    x = x.to("cuda:2")
                    prompt[i] = prompt[i].to("cuda:2")
                    x = prompt[i].reshape(B, 1, 1, -1) + x
                    x = blk(x)      

                elif 18 <= i < 27:
                    x = x.to("cuda:1")
                    prompt[i] = prompt[i].to("cuda:1")
                    x = prompt[i].reshape(B, 1, 1, -1) + x
                    x = blk(x)
                    
                else:
                    x = x.to("cuda:0")
                    prompt[i] = prompt[i].to("cuda:0")
                    x = prompt[i].reshape(B, 1, 1, -1) + x
                    x = blk(x)
            else:
                x = prompt[i].reshape(B, 1, 1, -1) + x
